...
public class AccSocketProtocol {
    ...
    public static byte[] buildRegisterRequest(String name, String password, int interval, String commandPassword) {
        ...
        outputStream.write(OutboundMessageTypes.REGISTER_COMMAND_APPLICATION);
        outputStream.write(BROADCASTING_PROTOCOL_VERSION);
        writeString(outputStream, name);
        writeString(outputStream, password);
        outputStream.write(toByteArray(interval, 4), 0, 4);
        writeString(outputStream, commandPassword);
        return outputStream.toByteArray();
    }

    public static byte[] buildUnregisterRequest(int connectionID) {
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        outputStream.write(OutboundMessageTypes.UNREGISTER_COMMAND_APPLICATION);
        outputStream.write(toByteArray(connectionID, 4), 0, 4);
        return outputStream.toByteArray();
    }
