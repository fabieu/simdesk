...
public interface PenaltyMapper {
    ...
    @Select("SELECT * FROM penalties WHERE session_id = #{sessionId} AND car_id = #{carId} ORDER BY id")
    List<Penalty> findBySessionAndCarId(int sessionId, int carId);

    @Insert("INSERT INTO penalties (session_id, car_id, reason, penalty, penalty_value, violation_lap, cleared_lap, post_race) " +
