...
public interface DiscordUserMapper {
    ...
    @Insert("""
            INSERT INTO users_discord (user_id, username, global_name, permits, update_datetime)
            VALUES (#{userId}, #{username}, #{globalName},#{permits, typeHandler=de.sustineo.simdesk.entities.typehandler.PermitsTypeHandler}, #{updateDatetime})
            ON CONFLICT(user_id)
                DO UPDATE SET username = #{username}, global_name = #{globalName}, update_datetime = #{updateDatetime}, permits = coalesce(#{permits, typeHandler=de.sustineo.simdesk.entities.typehandler.PermitsTypeHandler}, permits)
            """)
