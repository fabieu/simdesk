...
public interface RoundSessionMapper {
    ...
    @Select("SELECT * FROM stewarding_session WHERE round_id = #{roundId} ORDER BY sort_order")
    List<RoundSession> findByRoundId(String roundId);

    @ResultMap("roundSessionResultMap")
    @Select("SELECT * FROM stewarding_session WHERE id = #{id}")
    RoundSession findById(String id);

    @Insert("""
            INSERT INTO stewarding_session (id, round_id, session_type, title, start_time, end_time, sort_order, created_at)
            VALUES (#{id}, #{roundId}, #{sessionType}, #{title}, #{startTime}, #{endTime}, #{sortOrder}, CURRENT_TIMESTAMP)
            """)
    void insert(RoundSession session);

    @Update("""
            UPDATE stewarding_session
            SET round_id = #{roundId}, session_type = #{sessionType}, title = #{title},
